...
from .config import DET_PATH, REC_PATH, DICT_PATH
class GENInferencer(object):
    def __init__(self, det_path=DET_PATH, rec_path=REC_PATH, dict_path=DICT_PATH, rec_batch_size=16, time_profile=False, mem_profile=False ,**kwargs):
        super().__init__()
    ...
    def process_image(self, img):
        ...
        return box_list, texts, score_list # , debug_img

    def predict(self, img):
        return self.process_image(img)

    def predict_mem_profile(self, img):
        wapper = profile(self.process_image)
        return wapper(img)
    ...
    def predict_time_profile(self, img):
        ...
        for i in range(8):
            ...
            lp = LineProfiler()
            lp_wrapper = lp(self.process_image)
            ret = lp_wrapper(img)
            lp.print_stats()
        return ret
...
if __name__ == '__main__':
    ...
    model = GENInferencer(**args)
    txts, boxes = model.predict(img)
    print(txts, boxes)
