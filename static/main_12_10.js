...
async function startPlayback() {
  ...
  if (isPlaying) return alert("视频已在播放中");
  ...
  try {
    const res = await fetch(`/ctrl_playing?on=1`, { timeout: 10000 });
...
async function stopPlayback() {
    if (!isPlaying) return alert("视频未在播放");
...
function bindToggleResultBtn() {
    ...
    if (isResultVisible) {
        toggleResultBtn.innerHTML = '<i class="fa fa-eye-slash mr-1"></i>显示结果';
        resultOverlayCanvas.classList.remove('hidden');         // 显示叠加层
        redrawResultOverlay(); // 重新绘制（确保同步最新表格编辑）
    } else {
        toggleResultBtn.innerHTML = '<i class="fa fa-eye mr-1"></i>隐藏结果';
        resultOverlayCanvas.classList.add('hidden'); // 隐藏叠加层
    }
}
